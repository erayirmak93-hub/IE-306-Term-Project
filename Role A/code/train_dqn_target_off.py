...
def train_dqn_target_off():
    env = gym.make("DroneDispatch-v0")

    agent = DQNAgent()

    # Target network is intentionally not used in this ablation.
    ...
    for episode in range(NUM_EPISODES):
        ...
        while not done:
            ...
            if len(replay_buffer) >= BATCH_SIZE:
                ...
                actions = torch.tensor(actions, dtype=torch.long).unsqueeze(1).to(agent.device)
                ...
                next_action_masks = torch.tensor(next_action_masks, dtype=torch.float32).to(agent.device)

                q_values = agent.policy_net(states).gather(1, actions)

                with torch.no_grad():
                    # Target Network OFF:
                    # The online network is used for both action-value prediction
                    # and target value estimation.
                    next_q_all = agent.policy_net(next_states)

                    next_q_all[next_action_masks == 0] = -1e9

                    next_q_values = next_q_all.max(1, keepdim=True)[0]
    ...
    torch.save(
        ...
        os.path.join(weights_dir, "dqn_target_off_50ep.pt")
    )

    print("Training finished.")
    print("Model saved to weights/dqn_target_off_50ep.pt")


if __name__ == "__main__":
    train_dqn_target_off()