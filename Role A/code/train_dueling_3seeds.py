...
def train_one_seed(seed):
    ...
    target_net = DuelingDQNNetwork().to(agent.device)
        ...
        while not done:
            ...
            if len(replay_buffer) >= BATCH_SIZE:
                ...
                with torch.no_grad():
                    ...
                    next_q_values = next_q_target.gather(1, best_next_actions)
    ...
    csv_path = os.path.join(logs_dir, f"dueling_seed{seed}.csv")
    ...
    torch.save(
        ...
        os.path.join(weights_dir, f"dueling_seed{seed}.pt")
    )

    print(f"Seed {seed} finished.")
    print(f"Log saved to logs/dueling_seed{seed}.csv")
    print(f"Model saved to weights/dueling_seed{seed}.pt")
...
def main():
    ...
    print("All 3 seed runs finished.")
